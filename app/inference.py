...
MODEL_SEARCH_PATHS = [Path("models/best.pt")]
...
class InferenceEngine:
    ...
    def detect(self, frame: np.ndarray) -> List[Detection]:
        ...
        for d in deduped:
            print(
                f"    {d.name}: conf={d.confidence:.4f}  bbox={
                    [round(v, 3) for v in d.bbox]
                }"
            )
...
def encode_frame_with_detections(frame: np.ndarray, detections) -> str:
    ...
    for detection in detections or []:
        ...
        name = (
            ...
            else detection.get("name", "Tool")
        )
        confidence = (
            detection.confidence
            if hasattr(detection, "confidence")
            else float(detection.get("confidence") or 0)
        )
        x1 = int(bbox[0] * w)
        y1 = int(bbox[1] * h)
        x2 = int(bbox[2] * w)
        ...
        label_y = max(y1, lh + 8)
        cv2.rectangle(
            viz, (x1, label_y - lh - 8), (x1 + lw + 4, label_y), (0, 255, 140), -1
        )
        cv2.putText(
            viz,
            label,
